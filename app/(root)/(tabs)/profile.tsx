...
const Profile = () => {
  ...
  const handleAppeal = async () => {
    try {
      const res = await createAppealNotice({
        profileId: user?.profile?.$id ?? "",
      });
      if (res) Alert.alert("Success", "Sent an appeal for the admin!");
    } catch (error) {
      console.log(error);
  ...
  const handleRequestSport = async () => {
    Alert.prompt(
      "Enter Sport Name",
      "Please enter the name of the new sport you would like to request to the admin to add",
      [
        {
          text: "Cancel",
          style: "cancel",
        ...
        {
          text: "Confirm",
          onPress: async (newSport: any) => {
            try {
              if (!user?.role) return;

              const res = await createSportSubmission({
                profileId: user?.profile?.$id ?? "",
                newSport,
                role: user.role,
              });
              if (res)
                Alert.alert(
                  "Success",
                  "Sent a sport submission for the admin!"
                );
            } catch (error) {
              console.log(error);
              Alert.alert("Error", "Failed to send sport submission");
            }
          },
        },
      ]
